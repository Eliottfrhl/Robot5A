...
public:
    MoveItControlGui(rclcpp::Node::SharedPtr node)
        : node_(node), 
          ...
          tf_buffer_(node_->get_clock(), tf2::Duration(0)),  // Pass clock and zero duration
          tf_listener_(tf_buffer_)
    {
        ...
        connect(move_to_predefined_btn, &QPushButton::clicked, this, [this]() {
            moveToPosition(-0.018829, -0.252053, 0.578420, -0.303061, -0.566473, 0.624680, 0.443889);  // Predefined position
        });
...
private:
    ...
    void moveToPosition(double x, double y, double z, double qx, double qy, double qz, double qw) {
        RCLCPP_INFO(node_->get_logger(), "Moving to position: x=%.2f, y=%.2f, z=%.2f", x, y, z);
