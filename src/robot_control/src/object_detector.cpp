...
/// @brief Node that detects ArUco markers in images and broadcasts their poses
/// as TF transforms.
...
private:
  ...
  void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    if (!markerCorners.empty()) {
      ...
      cv::cornerSubPix(gray, markerCorners[0], cv::Size(5, 5), cv::Size(-1, -1),
                       criteria);
    }
    ...
    if (!markerIds.empty()) {
      ...
      cv::aruco::estimatePoseSingleMarkers(
          markerCorners, markerLength, camMatrix_, distCoeffs_, rvecs, tvecs);
