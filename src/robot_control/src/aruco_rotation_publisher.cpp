...
public:
    ...
    : Node("aruco_rotation_publisher")
    {
        ...
        for (int i = 0; i <= 15; ++i)
        {
            ...
            auto publisher = this->create_publisher<geometry_msgs::msg::Vector3Stamped>(topic_name, 10);
    ...
    void timer_callback()
    {
        ...
        for (int marker_id = 0; marker_id <= 15; ++marker_id)
        {
            ...
            try
            {
                ...
                transformStamped = tf_buffer_->lookupTransform(target_frame, marker_frame, tf2::TimePointZero);
                ...
                RCLCPP_DEBUG(this->get_logger(), "Marker %d: Roll=%f, Pitch=%f, Yaw=%f", marker_id, roll, pitch, yaw);
            }
            catch (tf2::TransformException &ex)
            {
                // Transform not available, skip this marker
                RCLCPP_DEBUG(this->get_logger(), "Could not get transform for marker %d: %s", marker_id, ex.what());
                continue;
            }
        }
    }
